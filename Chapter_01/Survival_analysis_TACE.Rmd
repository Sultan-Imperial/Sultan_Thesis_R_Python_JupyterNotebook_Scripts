---
title: "Survival analysis in patients with HCC treated with TACE Subset Group (n=103)"
...
- bib/references.bib
- bib/packages.bib
csl: "/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Scripts/bib/sage-harvard.csl"
logo: "/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Scripts/Images/Liver Ink.png"
editor_options:
...
# [) 🧬 Introduction to HCC and TACE-based treatment]{style="color:#000000; font-weight:bold;"} {.tabset .tabset-fade .tabset-pills}
...
![](/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Scripts/Images/TACE.gif){width="900px"}
:::

<div>

This script serves as the analytical backbone of my PhD thesis, processing targeted-panel next-generation sequencing data from a cohort of hepatocellular carcinoma (HCC) patients who have undergone transarterial chemo-embolisation (TACE). Beginning with raw FASTQ files, the pipeline performs quality control, adapter trimming, and alignment to the GRCh38 reference genome, followed by rigorous post-alignment filtering to eliminate PCR duplicates and low-quality reads. Variant calling is executed. Subsequent annotation integrates multiple databases (COSMIC, ClinVar, gnomAD, OncoKB) to contextualise each variant’s clinical relevance and functional impact. The script then applies custom R functions to correlate molecular profiles with overall survival (OS) and pregression-free survival (PFS) after TACE. Interactive visualisations—waterfall plots, lollipop plots, and Kaplan-Meier curves—are automatically produced to facilitate rapid interpretation.
...
1️⃣  **End-point definition**

• Overall survival (OS): time from first TACE session to death from any cause.

• OS status is defined as 1 for death and 0 for alive at last follow-up.

• Progression-free survival (PFS): time from first TACE session to radiological progression, or death, whichever occurs first.

• PFS status is defined as 1 for progression or death and 0 for alive and/or progression-free at last follow-up.

Patients alive and/or progression-free at last follow-up are right-censored.

2️⃣ **Descriptive exploration**

• Survival functions are estimated with Kaplan–Meier methodology, reporting median OS/PFS and their 95 % confidence intervals.        
<div style="margin-left: 300px;">
  `r knitr::include_graphics("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Scripts/Images/OS_PFS.gif")`
</div>
• Differences between strata (e.g., mutational status, BCLC stage, AFP level) are compared with the two-sided log-rank test.

3️⃣ **Univariable risk assessment**

• Cox proportional hazards (PH) models are fitted for each clinical, radiological, and molecular variable to assess their association with OS and PFS.

• Hazard ratios (HRs) with 95 % confidence intervals (CIs) and Wald P-values are reported.

4️⃣ **Multivariable modelling**

• A multivariable Cox PH model is constructed to identify independent prognostic factors for OS and PFS.

• Backward elimination (exit threshold P = 0.05) refines the model; stepwise selection and LASSO penalties are explored in sensitivity analyses to ensure robustness.

5️⃣ **Software environment**

• Analyses are executed in R (v4.4.2). Key packages: survival, survminer, rms, glmnet, mice, timeROC.

</div>
:::::


# [) ⚙️ Data Preparation and Loading Packages]{style="color:#000000; font-weight:bold;"} {.tabset .tabset-fade}

## [Load and Inspect Data]{style="color:#765543; font-weight:bold;"}

```{r Load_Data, echo=TRUE, cache=TRUE}
# Load the data
Survival_R <- read.csv("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Tables/Final_PFS_OS_TACE.csv")
...
```{r load-packages, echo=TRUE, cache=TRUE, include=TRUE}
...
cran_packages <- c("survival", "lubridate", "ggfortify", "flexsurv", 
...
create_whole_cohort_survival_plot <- function(surv_fit, title, time_col, status_col, ylab, annotation_x, annotation_y) {
  ...
  surv_plot <- ggsurvplot(surv_fit,
                         ...
                         tables.theme = theme_survminer(font.size = 14))  # Single color for whole cohort
  ...
  surv_plot$plot <- surv_plot$plot +
    annotate("label", x = annotation_x, y = annotation_y, label = label_text,
             size = 6, hjust = 0, vjust = 1, color = "black", fill = "lightblue")
...
create_survival_plot <- function(surv_fit, title, column_name, median_surv, time_col, status_col, ylab, annotation_x, annotation_y1, annotation_y2, annotation_y3, pval_y) {
  ...
  surv_plot <- ggsurvplot(surv_fit,
                          ...
                          tables.theme = theme_survminer(font.size = 14)
                          )
  # Create label texts
  ...
  surv_plot$plot <- surv_plot$plot +
    annotate("label", x = annotation_x, y = annotation_y1, label = label_A,
             size = 5, hjust = 0, vjust = 1, color = "black", fill = "#00BA38") +  # Green for Class A
    annotate("label", x = annotation_x, y = annotation_y2, label = label_B,
             size = 5, hjust = 0, vjust = 1, color = "black", fill = "#619CFF") +  # Blue for Class B
    annotate("label", x = annotation_x, y = annotation_y3, label = label_C,
             size = 5, hjust = 0, vjust = 1, color = "black", fill = "#F8766D") +  # Red for Class C
...
# [) 📉 Kaplan-Meier Survival Results]{style="color:#000000; font-weight:bold;"} {.tabset .tabset-fade .tabset-pills}
...
Whole_OS_plot <- create_whole_cohort_survival_plot(Whole_Cohort_OS, "Strata", 
                                                  "OS_Time", "OS_Status", 
                                                  "OS Probability", 60, 0.75)

# Display plot
print(Whole_OS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Whole_Cohort_OS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Whole_OS_plot)
dev.off()
```
...
**The median Overall Survival (OS)** for the whole cohort is **34.30 months (~ 3 years)**, but this figure on its own is only a point estimate derived from this specific sample of patients. To better understand the true prognosis for the entire population of similar patients, we look at **the 95% Confidence Interval (CI)**, which ranges **from 27.80 to 41.30 months**. This interval provides a plausible range for the true population median, indicating that while our best estimate is **34.3 months**, we can be 95% confident that the true median OS for this HCC cohort lies somewhere between approximately 28 to 41 months. The "95% confidence" itself is a statement about the reliability of the estimation procedure, suggesting that if this study were repeated many times, 95% of the resulting CIs would capture the actual median OS of the population. The breadth of this interval also gives us insight into the precision of our estimate; a wider range may reflect greater variability in the data or a smaller sample size, thus providing a more complete and realistic picture of the expected outcomes than the single median value alone.
...
Whole_PFS_plot <- create_whole_cohort_survival_plot(Whole_Cohort_PFS, "Strata", 
                                                   "PFS_Time", "PFS_Status", 
                                                   "PFS Probability", 29, 0.75)

# Display plot
print(Whole_PFS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Whole_Cohort_PFS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Whole_PFS_plot)
dev.off()
```
...
This plot shows us how long HCC patients live without their cancer getting worse after TACE treatment.
PFS measures the time from the start of treatment (TACE) until one of the following events occurs:

A) The HCC shows signs of progression (e.g., tumors grow in size or new tumors appear).
B) The patient dies from any cause.
...
<span style="color:#6638f0; font-weight:bold;">The central finding here:</span> is the median PFS of **12.43 months**. This means that after TACE treatment, half of the patients lived for approximately <span style="color:#049840; font-weight:bold;">**one year**</span> before their disease progressed. The dashed lines clearly illustrate how this point is determined, linking the 50% probability mark to the corresponding time.
...
BCLC_OS_plot <- create_survival_plot(BCLC_OS, "BCLC", "BCLC", median_os, "OS_Time", "OS_Status", 
                                    "OS Probability", 74, 1, 0.79, 0.58, 0.37)

# Display plot
print(BCLC_OS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/BCLC_OS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(BCLC_OS_plot)
dev.off()
```
...
<img src="/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Scripts/Images/BCLC2.png" width="400px" height="400px" alt="BCLC Stage 2">
<p><em>Figure 1</em></p>
</div>
</div>
...
<span style="color:#5cc9f5; font-weight:bold;">**Key Findings:**</span>


<span style="color:#028236; font-weight:bold;">**Class A:**</span> This early-stage group has the best prognosis. The median OS is 60.73 months (about 5 years). The confidence interval is "NA" (Not Available/Not Applicable) on the upper end because fewer than 50% of the patients in this group had died by the end of the follow-up period, which is a very positive sign ✅.
...
BCLC_PFS_plot <- create_survival_plot(BCLC_PFS, "BCLC", "BCLC", median_pfs, "PFS_Time", "PFS_Status", 
                                     "PFS Probability", 36, 1, 0.79, 0.58, 0.37)

# Display plot
print(BCLC_PFS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/BCLC_PFS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(BCLC_PFS_plot)
dev.off()
```
...
::: {style="font-weight:bold; color:green; margin-bottom:5px;"}
This plot shows the probability of patients being alive and their cancer not having progressed, grouped by their initial BCLC stage. This is a measure of how long the initial treatment (like TACE) remains effective.
:::
<span style="color:#5cc9f5; font-weight:bold;">**Key Findings:**</span>
...
<span style="color:#028236; font-weight:bold;">**Class A:**</span> Has the longest period of disease control, with a median PFS of 21.30 months .
<span style="color:#0843A9; font-weight:bold;">**Class  B:**</span> Has an intermediate median PFS of 11.53 months.
<span style="color:#A90813; font-weight:bold;">**Class  C:**</span> Has the shortest period of disease control, with a median PFS of only 5.83 months.
...
The PFS data clearly and significantly (p=0.0128) demonstrates that BCLC staging is a **prognostic indicator 🔍** for tumour (HCC) control following TACE.
:::



<span style="color:#0516D3; font-weight:bold;">**Clinical Interpretation:**</span>
...
<span style="color:#909703; font-weight:bold;">**The Complete Story Comparing the two plots tells a powerful story:**</span>
...
# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/CTP_OS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(CTP_OS_plot)
dev.off()
```
...
# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/CTP_PFS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(CTP_PFS_plot)
dev.off()
```
...
# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/PS_OS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(PS_OS_plot)
dev.off()
```
...
# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/PS_PFS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(PS_PFS_plot)
dev.off()
```
...
# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/HAP_OS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(HAP_OS_plot)
dev.off()
```
...
# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/HAP_PFS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(HAP_PFS_plot)
dev.off()
```
...
# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Cirrhosis_OS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Cirrhosis_OS_plot)
dev.off()
```
...
# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Cirrhosis_PFS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Cirrhosis_PFS_plot)
dev.off()
```
...
# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Viral_OS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Viral_OS_plot)
dev.off()
```
...
# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Viral_PFS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Viral_PFS_plot)
dev.off()
```
...
Tumour_OS_plot <- create_survival_plot_binary(Tumour_OS, "Tumour Size", "Tumour_size", median_os, "OS_Time", "OS_Status", 
                                  "OS Probability", 80, 0.95, 0.75, 0.55, "<5 cm", "≥5 cm")

# Display plot
print(Tumour_OS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Tumour_OS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Tumour_OS_plot)
dev.off()
```
...
Tumour_PFS_plot <- create_survival_plot_binary(Tumour_PFS, "Tumor Size", "Tumour_size", median_pfs, "PFS_Time", "PFS_Status", 
                                   "PFS Probability", 40, 0.97, 0.77, 0.57, "<5 cm", "≥5 cm")

# Display plot
print(Tumour_PFS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Tumour_PFS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Tumour_PFS_plot)
dev.off()
```
...
AFP_OS_plot <- create_survival_plot_binary(AFP_OS, "AFP", "AFP_binary", median_os, "OS_Time", "OS_Status", 
                                  "OS Probability", 80, 0.95, 0.75, 0.55, "<400 ng/mL", "≥400 ng/mL")

# Display plot
print(AFP_OS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/AFP_OS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(AFP_OS_plot)
dev.off()
```
...
AFP_PFS_plot <- create_survival_plot_binary(AFP_PFS, "AFP", "AFP_binary", median_pfs, "PFS_Time", "PFS_Status", 
                                   "PFS Probability", 40, 0.97, 0.77, 0.57, "<400 ng/mL", "≥400 ng/mL")

# Display plot
print(AFP_PFS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/AFP_PFS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(AFP_PFS_plot)
dev.off()
```
...
Albumin_OS_plot <- create_survival_plot_binary(Albumin_OS, "Albumin", "Albumin_binary", median_os, "OS_Time", "OS_Status", 
                                  "OS Probability", 80, 0.95, 0.75, 0.55, "<36 g/L", "≥36 g/L")

# Display plot
print(Albumin_OS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Albumin_OS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Albumin_OS_plot)
dev.off()
```
...
Albumin_PFS_plot <- create_survival_plot_binary(Albumin_PFS, "Albumin", "Albumin_binary", median_pfs, "PFS_Time", "PFS_Status", 
                                   "PFS Probability", 40, 0.97, 0.77, 0.57, "<36 g/L", "≥36 g/L")

# Display plot
print(Albumin_PFS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Albumin_PFS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Albumin_PFS_plot)
dev.off()
```
...
Bilirubin_OS_plot <- create_survival_plot_binary(Bilirubin_OS, "Bilirubin", "Bilirubin_binary", median_os, "OS_Time", "OS_Status", 
                                  "OS Probability", 80, 0.95, 0.75, 0.55, "<11 μmol/L", "≥11 μmol/L")

# Display plot
print(Bilirubin_OS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Bilirubin_OS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Bilirubin_OS_plot)
dev.off()
```
...
Bilirubin_PFS_plot <- create_survival_plot_binary(Bilirubin_PFS, "Bilirubin", "Bilirubin_binary", median_pfs, "PFS_Time", "PFS_Status", 
                                   "PFS Probability", 40, 0.97, 0.77, 0.57, "<11 μmol/L", "≥11 μmol/L")

# Display plot
print(Bilirubin_PFS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Bilirubin_PFS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Bilirubin_PFS_plot)
dev.off()
```
...
# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Response_OS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Response_OS_plot)
dev.off()
```
...
# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Response_PFS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Response_PFS_plot)
dev.off()
```
...
Age_OS_plot <- create_survival_plot_binary(Age_OS, "Age", "Age_binary", median_os, "OS_Time", "OS_Status", 
                                  "OS Probability", 80, 0.95, 0.75, 0.55, "<69 years", "≥69 years")

# Display plot
print(Age_OS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Age_OS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Age_OS_plot)
dev.off()
```
...
Age_PFS_plot <- create_survival_plot_binary(Age_PFS, "Age", "Age_binary", median_pfs, "PFS_Time", "PFS_Status", 
                                   "PFS Probability", 40, 0.97, 0.77, 0.57, "<69 years", "≥69 years")

# Display plot
print(Age_PFS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/Age_PFS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(Age_PFS_plot)
dev.off()
```
...
# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/PVT_OS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(PVT_OS_plot)
dev.off()
```
...
# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/PVT_PFS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(PVT_PFS_plot)
dev.off()
```
...
cfDNA_OS_plot <- create_survival_plot_binary(cfDNA_OS, "cfDNA Level", "cfDNA", median_os, "OS_Time", "OS_Status", 
                                  "OS Probability", 80, 0.95, 0.75, 0.55, "<544 ng/mL", "≥544 ng/mL")

# Display plot
print(cfDNA_OS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/cfDNA_OS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(cfDNA_OS_plot)
dev.off()
```
...
cfDNA_PFS_plot <- create_survival_plot_binary(cfDNA_PFS, "cfDNA Level", "cfDNA", median_pfs, "PFS_Time", "PFS_Status", 
                                   "PFS Probability", 40, 0.97, 0.77, 0.57, "<544 ng/mL", "≥544 ng/mL")

# Display plot
print(cfDNA_PFS_plot)

# Save plot
png("/Users/sultanalharbi/Library/CloudStorage/OneDrive-Personal/Projects/Thesis_Chapters/Chapter 1 (cfDNA-TACE for HCC)/TACE_Results/cfDNA_PFS_TACE.png", width = 10, height = 8, units = "in", res = 300)
print(cfDNA_PFS_plot)
dev.off()
```

# [) 📈 Cox Regression Survival Results]{style="color:#000000; font-weight:bold;"} {.tabset .tabset-fade .tabset-pills}
...
(cox_model_OS_AFP <- coxph(Surv(OS_Time, OS_Status) ~ AFP_binary, data = Survival_R) %>%
  ...
  modify_header(label = "Effect of AFP (ng/mL) level > 400 'high' versus ≤ 400 'low' on OS"))
...
(cox_model_PFS_AFP <- coxph(Surv(PFS_Time, PFS_Status) ~ AFP_binary, data = Survival_R) %>%
  tbl_regression(exp = TRUE) %>%
  modify_header(label = "Effect of AFP (ng/mL) level > 400 'high' versus ≤ 400 'low' on PFS"))
```
...
(cox_model_OS_Albumin <- coxph(Surv(OS_Time, OS_Status) ~ Albumin_binary, data = Survival_R) %>%
  tbl_regression(exp = TRUE) %>%
  modify_header(label = "Effect of Albumin (g/L) level < 36 'low' versus ≥ 36 'normal' on OS"))

# Fit the Cox proportional hazards model for Progression-Free Survival (PFS)
(cox_model_PFS_Albumin <- coxph(Surv(PFS_Time, PFS_Status) ~ Albumin_binary, data = Survival_R) %>%
  tbl_regression(exp = TRUE) %>%
  modify_header(label = "Effect of Albumin (g/L) level < 36 'low' versus ≥ 36 'normal' on PFS"))
```
...
(cox_model_OS_Bilirubin <- coxph(Surv(OS_Time, OS_Status) ~ Bilirubin_binary, data = Survival_R) %>%
  tbl_regression(exp = TRUE) %>%
  modify_header(label = "Effect of Bilirubin (μmol/L) level < 11 'normal' versus ≥ 11 'elevated' on OS"))

# Fit the Cox proportional hazards model for Progression-Free Survival (PFS)
(cox_model_PFS_Bilirubin <- coxph(Surv(PFS_Time, PFS_Status) ~ Bilirubin_binary, data = Survival_R) %>%
  tbl_regression(exp = TRUE) %>%
  modify_header(label = "Effect of Bilirubin (μmol/L) level < 11 'normal' versus ≥ 11 'elevated' on PFS"))
```
...
(cox_model_OS_Age <- coxph(Surv(OS_Time, OS_Status) ~ Age_binary, data = Survival_R) %>%
  tbl_regression(exp = TRUE) %>%
  modify_header(label = "Effect of Age < 69 'younger' versus ≥ 69 'older' on OS"))

# Fit the Cox proportional hazards model for Progression-Free Survival (PFS)
(cox_model_PFS_Age <- coxph(Surv(PFS_Time, PFS_Status) ~ Age_binary, data = Survival_R) %>%
  tbl_regression(exp = TRUE) %>%
  modify_header(label = "Effect of Age < 69 'younger' versus ≥ 69 'older' on PFS"))
```
...
(cox_model_OS_Tumour <- coxph(Surv(OS_Time, OS_Status) ~ Tumour_size, data = Survival_R) %>%
  tbl_regression(exp = TRUE) %>%
  modify_header(label = "Effect of Tumor Size: < 5 cm 'small' versus ≥ 5 cm 'large' on OS"))

# Fit the Cox proportional hazards model for Progression-Free Survival (PFS)
(cox_model_PFS_Tumour <- coxph(Surv(PFS_Time, PFS_Status) ~ Tumour_size, data = Survival_R) %>%
  tbl_regression(exp = TRUE) %>%
  modify_header(label = "Effect of Tumor Size: < 5 cm 'small' versus ≥ 5 cm 'large' on PFS"))
```
...
(cox_model_OS_cfDNA <- coxph(Surv(OS_Time, OS_Status) ~ cfDNA, data = Survival_R) %>%
  tbl_regression(exp = TRUE) %>%
  modify_header(label = "Effect of cfDNA Level: < 544 ng/mL 'low' versus ≥ 544 ng/mL 'high' on OS"))

# Fit the Cox proportional hazards model for Progression-Free Survival (PFS)
(cox_model_PFS_cfDNA <- coxph(Surv(PFS_Time, PFS_Status) ~ cfDNA, data = Survival_R) %>%
  tbl_regression(exp = TRUE) %>%
  modify_header(label = "Effect of cfDNA Level: < 544 ng/mL 'low' versus ≥ 544 ng/mL 'high' on PFS"))
```
...
```{r Stepwise_Model_pValue, echo=TRUE, cache=TRUE}
# Stepwise model selection for OS
full_model_OS_pValue <- coxph(Surv(OS_Time, OS_Status) ~ AFP_binary + BCLC + HAP + PVT + Tumour_size, 
                     data = Survival_R)

step_model_OS_pvalue <- step(full_model_OS_pValue, direction = "both", trace = 0)
(step_model_OS_pvalue %>%
  tbl_regression(exp = TRUE) %>%
  modify_header(label = "Stepwise Selected Model for Overall Survival"))

# Stepwise model selection for PFS
full_model_PFS_pvalue <- coxph(Surv(PFS_Time, PFS_Status) ~ AFP_binary + BCLC + HAP + PVT + Tumour_size, 
                      data = Survival_R)

step_model_PFS_pvalue <- step(full_model_PFS_pvalue, direction = "both", trace = 0)
(step_model_PFS_pvalue %>%
  tbl_regression(exp = TRUE) %>%
  modify_header(label = "Stepwise Selected Model for Progression-Free Survival"))
```
